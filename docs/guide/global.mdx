...
Bilibili app所使用的`gRPC接口`已移除`HTTP/1.1兼容性`，仅保留`HTTP/2协议`支持  
不支持`MitM over HTTP/2`的 VPN 不再可以使用本脚本  
...
<ModuleInstall urlPrefix="https://github.com/BiliUniverse/Global/raw/main/modules/">
  ...
  <ModuleInstall.Tab type="qx">
    <ModuleInstall.Item url="BiliBili.Global.snippet" badge={<>需要 1.4.0 (783) 及以上版本</>}>
      - ❌ 不再支持`Quantumult X` (`Quantumult X`不支持MitM over HTTP/2)
  ...
  <ModuleInstall.Tab type="shadowrocket">
    <ModuleInstall.Item url="BiliBili.Global.srmodule" badge={<>需要 2.2.29 (1925) 及以上版本</>}>
    - ❌ 不再支持`Shadowrocket` (`Shadowrocket`不支持MitM over HTTP/2)
